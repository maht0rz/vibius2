...
class View{
    ...
    public function load($name){
        $this->file = dirname(__FILE__).'/../app/views/'.$name.'.tpl.html';
        $GLOBALS['debugger_template'] = $this->file;
        $this->template = true;
        if(!file_exists($this->file)){
            $this->template = false;
            $this->file = dirname(__FILE__).'/../app/views/'.$name.'.html';
            if(!file_exists($this->file)){
    ...
    private function parse($view){
          ...
          $re6='(\\})'; # Any Single Character 6     
                  ...
                  if ($c=preg_match_all ("/".$re1.$re4.$re6."/is", $view, $matches))
                  {
                    foreach($matches[2] as $match){
                        $view = preg_replace("/".$re1.'('.$match.')'.$re6."/is","<?php echo $".$match.";?>",$view);
                    }
                  }
/*
